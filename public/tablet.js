...
try {
    const socket = io();
// DOM Elements
const connectionDot = safeGetElementById('connection-dot');
const connectionText = safeGetElementById('connection-text');
const btnConnect = safeGetElementById('btn-connect');
const ipInput = safeGetElementById('matrix-ip');
const portInput = safeGetElementById('matrix-port');
const btnScan = safeGetElementById('btn-scan');
const extendedInput = safeGetElementById('matrix-extended');

const inputsCountInput = safeGetElementById('matrix-inputs');
const outputsCountInput = safeGetElementById('matrix-outputs');
const btnGenerate = safeGetElementById('btn-generate');
const matrixGrid = safeGetElementById('matrix-grid');

const layoutNameInput = safeGetElementById('layout-name');
...
const btnSortRows = safeGetElementById('btn-sort-rows');
const btnSortCols = safeGetElementById('btn-sort-cols');
const btnSortRowsId = safeGetElementById('btn-sort-rows-id');
const btnSortColsId = safeGetElementById('btn-sort-cols-id');

function saveAndRenderSort() {
    renderGrid();
    const activeName = safeGetElementById('saved-layouts').value || safeGetElementById('layout-name').value;
    if (activeName) {
...
const btnSaveLayout = safeGetElementById('btn-save-layout');
const btnLoadLayout = safeGetElementById('btn-load-layout');
const btnDeleteLayout = safeGetElementById('btn-delete-layout');
const savedLayoutsSelect = safeGetElementById('saved-layouts');

const selectedOutEl = safeGetElementById('selected-out');
const currentConnEl = safeGetElementById('current-connection');
const selectedInEl = safeGetElementById('selected-in');
const btnTake = safeGetElementById('btn-take');
const btnUndo = safeGetElementById('btn-undo');

const btnSortToggle = safeGetElementById('btn-sort-toggle');
if (btnSortToggle) {
...
const btnTrash = safeGetElementById('btn-trash');
    ...
    btnTrash.addEventListener('drop', (e) => {
        ...
        try {
            ...
            if (data.type === 'output') {
                ...
                safeGetElementById('matrix-outputs').value = numOutputs;
                safeGetElementById('matrix-inputs').value = numInputs;
                const activeNameOut = safeGetElementById('saved-layouts').value || safeGetElementById('layout-name').value;
                if (activeNameOut) {
                    const layoutsObj = JSON.parse(localStorage.getItem('probelLayouts') || '{}');
                ...
                safeGetElementById('matrix-outputs').value = numOutputs;
                safeGetElementById('matrix-inputs').value = numInputs;
                const activeNameIn = safeGetElementById('saved-layouts').value || safeGetElementById('layout-name').value;
                if (activeNameIn) {
                    const layoutsObj = JSON.parse(localStorage.getItem('probelLayouts') || '{}');
...
function updateRecentColorsDatalist() {
    let datalist = safeGetElementById('recent-colors');
    if (!datalist) {
...
const btnShutdown = safeGetElementById('btn-shutdown');
if (btnShutdown) {
...
btnGenerate.addEventListener('click', () => {
    ...
    for (let inIdx = 0; inIdx < numInputs; inIdx++) {
        for (let outIdx = 0; outIdx < numOutputs; outIdx++) {
            const cp = safeGetElementById(`cp-${outIdx}-${inIdx}`);
            if (cp) {
...
rowLabel.appendChild(dragHandleIn);
        ...
        for (let outIdx = 0; outIdx < numOutputs; outIdx++) {
            ...
            cp.addEventListener('mouseenter', () => {
                const hOut = safeGetElementById(`header-out-${outIdx}`);
                const hIn = safeGetElementById(`header-in-${inIdx}`);
...
// Export Layouts
const btnExportLayouts = safeGetElementById('btn-export-layouts');
const btnImportLayouts = safeGetElementById('btn-import-layouts');
const importFileInput = safeGetElementById('import-file-input');
...
function updateSchedulerDropdowns() {
    const selOut = safeGetElementById('schedule-out');
    const selIn = safeGetElementById('schedule-in');
...
function renderSchedules() {
    const list = safeGetElementById('schedules-list');
    if (!list) return;
    list.innerHTML = '';
...
const btnAddSched = safeGetElementById('btn-add-schedule');
if (btnAddSched) btnAddSched.addEventListener('click', () => {
    const hrVal = safeGetElementById('schedule-hour').value;
    const minVal = safeGetElementById('schedule-minute').value;
    const outVal = safeGetElementById('schedule-out').value;
    const inVal = safeGetElementById('schedule-in').value;
...
const warningModal = safeGetElementById('warning-modal');
const warningCountdown = safeGetElementById('warning-countdown');
const warningDetails = safeGetElementById('warning-details');
let currentWarningTask = null;

const btnCancelTake = safeGetElementById('btn-cancel-take');
...
const btnSaveSnapshot = safeGetElementById('btn-save-snapshot');
const btnRecallSnapshot = safeGetElementById('btn-recall-snapshot');
const btnDeleteSnapshot = safeGetElementById('btn-delete-snapshot');
const savedSnapshotsSelect = safeGetElementById('saved-snapshots');
const snapshotNameInput = safeGetElementById('snapshot-name');
const snapshotSection = safeGetElementById('snapshot-section');
...
const layoutButtonsList = document.getElementById('layout-buttons-list');

if (layoutButtonsList) {
    const refreshTabletLayouts = () => {
        layoutButtonsList.innerHTML = '';
        const layoutsObj = JSON.parse(localStorage.getItem('probelLayouts') || '{}');
        const layoutNames = Object.keys(layoutsObj);
        
        if (layoutNames.length === 0) {
            layoutButtonsList.innerHTML = '<div style="color: var(--text-muted); font-size: 0.9rem;">No layouts saved. Use PC App to create layouts.</div>';
            return;
        }

        layoutNames.forEach(name => {
            const btn = document.createElement('button');
            btn.className = 'btn-layout-recall';
            ...
            btn.addEventListener('click', () => {
                document.querySelectorAll('.btn-layout-recall').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                
                const layout = layoutsObj[name];
                if (layout) {
                    activeLayoutName = name;
                    numInputs = layout.numInputs || 1;
                    numOutputs = layout.numOutputs || 1;
                    inputsData = layout.inputsData || (layout.inputLabels ? layout.inputLabels.map((n, i) => ({name: n, id: i})) : []);
                    outputsData = layout.outputsData || (layout.outputLabels ? layout.outputLabels.map((n, i) => ({name: n, id: i})) : []);
                    
                    renderGrid();
                }
            });
            layoutButtonsList.appendChild(btn);
        });
    };
    
    refreshTabletLayouts();
    
    socket.on('layouts_list', () => {
        refreshTabletLayouts();
    });
}

// Tablet Snapshot Logic
const btnSaveSnapTablet = document.getElementById('btn-save-snap-tablet');
const btnRecallSnapTablet = document.getElementById('btn-recall-snap-tablet');
const modalSaveSnap = document.getElementById('modal-save-snap');
const modalRecallSnap = document.getElementById('modal-recall-snap');
const btnConfirmSaveSnap = document.getElementById('btn-confirm-save-snap');
const btnCancelSaveSnap = document.getElementById('btn-cancel-save-snap');
const btnCancelRecallSnap = document.getElementById('btn-cancel-recall-snap');
const tabletSnapName = document.getElementById('tablet-snap-name');
const tabletSnapList = document.getElementById('tablet-snap-list');

if (btnSaveSnapTablet) {
    btnSaveSnapTablet.addEventListener('click', () => {
        if (!activeLayoutName) return alert('Nessun layout caricato');
        tabletSnapName.value = '';
        modalSaveSnap.style.display = 'flex';
        setTimeout(() => tabletSnapName.focus(), 100);
    });

    if (btnCancelSaveSnap) btnCancelSaveSnap.addEventListener('click', () => {
        modalSaveSnap.style.display = 'none';
    });

    if (btnConfirmSaveSnap) btnConfirmSaveSnap.addEventListener('click', () => {
        const name = tabletSnapName.value.trim();
        if (!name) return alert('Inserisci un nome per lo snapshot');
        if (!activeLayoutName) return alert('Nessun layout caricato');
        ...
        const layoutsObj = JSON.parse(localStorage.getItem('probelLayouts') || '{}');
        const layout = layoutsObj[activeLayoutName];
        if (layout) {
            if (!layout.snapshots) layout.snapshots = {};
            layout.snapshots[name] = state;
            socket.emit('save_layout', { name: activeLayoutName, layout });
            modalSaveSnap.style.display = 'none';
        } else {
            alert('Layout not found locally. Please reload.');
        }
    });

    btnRecallSnapTablet.addEventListener('click', () => {
        if (!activeLayoutName) return alert('Nessun layout caricato');
        
        const layoutsObj = JSON.parse(localStorage.getItem('probelLayouts') || '{}');
        const layout = layoutsObj[activeLayoutName];
        tabletSnapList.innerHTML = '';
        
        if (layout && layout.snapshots && Object.keys(layout.snapshots).length > 0) {
            Object.keys(layout.snapshots).forEach(snapName => {
                const btn = document.createElement('button');
                btn.className = 'snap-btn';
                btn.textContent = snapName;
                ...
                btn.addEventListener('click', () => {
                    ...
                    previewingSnapshot = stateMap;
                    updateGridVisuals();
                    modalRecallSnap.style.display = 'none';
                });
                tabletSnapList.appendChild(btn);
            });
        } else {
            tabletSnapList.innerHTML = '<p style="color:white; text-align:center;">Nessuno snapshot salvato.</p>';
        }
        modalRecallSnap.style.display = 'flex';
    });

    if (btnCancelRecallSnap) btnCancelRecallSnap.addEventListener('click', () => {
        modalRecallSnap.style.display = 'none';
    });
}

// Tablet Navigation & Theme UI Handlers
const btnToggleSidebar = document.getElementById('btn-toggle-sidebar');
const tabletContainer = document.querySelector('.tablet-container');
if (tabletContainer) {
    const isSidebarCollapsed = localStorage.getItem('tabletSidebarCollapsed') === 'true';
    if (isSidebarCollapsed) {
        tabletContainer.classList.add('sidebar-collapsed');
    }
    if (btnToggleSidebar) {
        btnToggleSidebar.addEventListener('click', () => {
            tabletContainer.classList.toggle('sidebar-collapsed');
            localStorage.setItem('tabletSidebarCollapsed', tabletContainer.classList.contains('sidebar-collapsed'));
        });
    }
}
...
if (btnFullscreen) {
    ...
    document.addEventListener('fullscreenchange', () => {
        if (document.fullscreenElement) {
            btnFullscreen.textContent = '📺 Esci Full Screen';
        } else {
...
if (btnThemeToggle) {
    const isLightTheme = localStorage.getItem('tabletLightTheme') === 'true';
    if (isLightTheme) {
    ...
    btnThemeToggle.addEventListener('click', () => {
        ...
        localStorage.setItem('tabletLightTheme', currentIsLight);
        btnThemeToggle.textContent = currentIsLight ? '🌙' : '☀️';
    });
}
